...
\program{Curso de Tecnologia em Sistemas para Internet}
\programen{Undergradute Program in Technology in Internet Systems}
\degree{Tecnólogo}
\degreearea{Sistemas para Internet}
% Caso seja TSI:
...
% TCC2:
\goal{Monografia de Trabalho de Conclusão de Curso de Graduação}
\course{Trabalho de Conclusão de Curso 2}


% Dados do TCC (precisa alterar)
\author{VINICIUS FERREIRA NOVACOSKI}  % Seu nome
\authorbib{FERREIRA NOVACOSKI, VINICIUS} % Seu nome para referência bibliográfica (Sobrenome, Nome)
\title{Refatoração e Implementação de Novas Funcionalidades no E-Museu} % Título do trabalho
\titleen{Refactoring and Implementation of New Features in the E-Museu} % Título traduzido para inglês
\advisor{Dra. Sediane Carmem Lunardi Hernandes} % Nome do orientador. Lembre-se de prefixar com "Prof. Dr.", "Profª. Drª.", "Prof. Me." ou "Profª. Me."}
% Se não houver corientador, comente a linha a baixo
\coadvisor{Dr. Diego Marczal} % Nome do coorientador, caso exista. Caso não exista, comente a linha.
\depositshortdate{2026} % Ano em que depositou este documento
\approvaldate{00/null/0000}

% Dados do curso que não precisam de alteração
\university{Universidade Tecnológica Federal do Paraná}
\universityen{Federal University of Technology -- Paraná}
\universitycampus{Campus Guarapuava}
\universityunit{Departamento Acadêmico de Computação}
\address{GUARAPUAVA}
\addressen{Guarapuava, PR, Brazil}
...
\PalavraChaveA{museu}%% museu
\PalavraChaveB{refatora\c{c}\~ao}%% refatoração
\PalavraChaveC{funcionalidades}%% funcionalidades
\PalavraChaveD{engenharia de software}%% engenharia de software
\PalavraChaveE{sistema web}%% sistema web
...
\KeywordA{museum}
\KeywordB{refactoring}
\KeywordC{functionalities}
\KeywordD{software engineering}
\KeywordE{web development}
